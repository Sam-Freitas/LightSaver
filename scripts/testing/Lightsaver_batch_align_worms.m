...
prompt = {'Enter number of worms to detect: ', ...
    ...
    'Export Straightened worms - yes (1) - no (0)'};
dlgtitle = 'User Inputs for Lightsaver';
dims = [1 100];
definput = {'5','0','0','','1','1','1','0','1'};
...
export_straightened_worms = str2double(answer{9});
...
%             break
    ...
    % this is a check to make sure the thicken labels are still correct
    if (max(labeled_masks(:)) ~= number_worms_to_detect) ...
            && (max(this_label(:)) == number_worms_to_detect)
        labeled_masks = this_label;
    end
...
for i = 1:length(img_paths)
    ...
    img_paths_new{i} = fullfile(filepath,[new_name ext]);
    
    if ~isequal(img_paths{i},img_paths_new{i})
        movefile(img_paths{i},img_paths_new{i});
        disp(['renaming:' name ' to ' new_name])
    end
