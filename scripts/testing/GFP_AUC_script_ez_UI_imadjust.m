...
for i = 1:length(img_paths)
    ...
    data_norm = double(imadjust(data))/255;
    
    erode_mask = imerode(data_norm>mean2(data_norm),se);
    erode_mask = imfill(erode_mask,'holes');
    erode_mask = imclose(erode_mask,strel('disk',1));
    
    data_norm = data_norm.*erode_mask;
