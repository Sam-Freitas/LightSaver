...
prompt = {'Enter number of worms to detect: ', ...
    ...
    'Remove 001,002, etc, from tif names - yes(1) - no(0)'};
...
output_path = fullfile(erase(erase(erase(pwd,'testing'),'GFP_AUC_script.m'),'scripts'),'exported_images');
mkdir(output_path);
...
img_paths2 = img_paths;

for i = 1:length(img_paths2)
    
    img_paths2{i} = erase(img_paths2{i},{'001',...
        '002','003','004','005','006','007','008','009',});
    
    if ~isequal(img_paths{i},img_paths2{i})
        movefile(img_paths{i},img_paths2{i});
    end
