...
model.load_state_dict(
    torch.load(
        r"C:\Users\LabPC2\Documents\GitHub\LightSaver\scripts_python\network_testing\trained_weights_mask_fixer\model_20260305_132941_training.pt"
        , weights_only = True)) #### uncomment this to use a previously trained weights 
model.eval()

image_paths = natsorted(glob.glob(os.path.join(testing_path,'*.png')))
...
with torch.no_grad():
    ...
    for i, tinputs in enumerate(tqdm.tqdm(testing_loader)):
        ...
        if tinputs.shape[0] > 1:
            ...
            write_outputs_to_images(tinputs, 1*toutputs, output_path, i = img_name , binary_threshold = testing_binary_threshold, export_inputs_and_outputs=False)
