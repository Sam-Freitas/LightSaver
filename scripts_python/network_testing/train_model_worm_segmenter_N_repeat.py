...
def data_transforms(p=0):
    ...
    this_data_transform = A.Compose([
        ...
        RandomBlankORNoise(p=p),

        # add blobs that cancel out the image pieces
        RandomBlobOverlay(p=p),

        # add a self overlayed image for worms that overlap themselves
        RotateSelfOverlay(p=p,angle_limit=360),
...
class RandomBlobNoise(A.ImageOnlyTransform):
    ...
    def get_blob_thresh(self,bounds):
    ...
    def apply(self, img, **params):
        ...
        this_blob_thresh = self.get_blob_thresh(self.blob_size)
        ...
        thresh = cv2.threshold(stretch, this_blob_thresh, 255, cv2.THRESH_BINARY)[1]
        ...
        blurred_mask = ((blured_mask.astype(img.dtype))/255)*(np.mean(img) + (0.5)*np.std(img))#*np.max(img)
...
class RandomBlobOverlay(A.ImageOnlyTransform):
    def __init__(self, blob_size = [100,210], p = 0.3):
        super(RandomBlobOverlay, self).__init__(p=p)
        self.blob_size = blob_size
    ...
    def apply(self, img, **params):
        ...
        img[mask] = np.mean(img[np.logical_and(img<(np.median(img)+0.001),img>(np.median(img)-0.001))])

        # add mask to input
        img = np.clip(img,a_max=initial_img_max, a_min = 0)
        pass

        return img

class RandomBlankORNoise(A.DualTransform):
    def __init__(self,p=0.5):
        super().__init__(p=p)
        self.p = p

    def get_blank_or_noise(self):
    ...
    def apply(self, img, **params):
        ...
        if self.get_blank_or_noise():
            out = np.zeros_like(img)
        else:
            out = (np.random.rand(*img.shape)*img.max()).astype(img.dtype)
