...
for i = 1:length(img_paths)
    ...
    se = strel('disk',100);
    background = imopen(data,se);
    
    filt_data = double(data).*(data>background);
    
    lin_data = nonzeros(double(data).*(filt_data>median(filt_data(:))));
    ...
    try
        this_mask = data>local_mins(1);
        this_mask = bwareaopen(this_mask,1000);
        
        this_label = bwlabel(this_mask);
        
        if max(this_label(:)) == number_worms_to_detect
            disp(['Found ' num2str(number_worms_to_detect) ' blobs'])
        else
            disp('trying again')
            error('trying mathmatical method')
        end
